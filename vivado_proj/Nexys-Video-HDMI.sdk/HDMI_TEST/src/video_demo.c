...
void store_image_to_buffer_and_ip_buffer(u8* in_buffer, u32* edge_ip_input_buffer_left, u32* edge_ip_input_buffer_right, u32* edge_ip_output_buffer, int current_image_index, u8* full_image_buffer){
  int is_first = current_image_index == 0;
  int is_last = current_image_index == NUM_IMAGES - 1;
...
void convert_centres_and_edges_to_row_major(u8** centre_buffers_pointers, u8** edge_buffers_pointers, u8** full_image_buffer){
  ...
  for(int i = 0; i < NUM_IMAGES; i++) {
	...
	for (int row = 0; row < 480; row++){
	  ...
	  int edge_right = edge_column_offset;
	  int edge_left = edge_column_offset + 3 * 480 * BLUR_WIDTH;
...
int delta_angle() {
	int16_t zAxis = 0;
	while (GYRO_Int2Status(&gyroDevice) == 0);
...
int main(void)
{
	...
	int status;
	status = XGpio_Initialize(&BTNInst, XPAR_GPIO_0_DEVICE_ID); // Replace with your actual GPIO ID
	if (status != XST_SUCCESS) {
